function $DetourProvider(
  ...
  State.prototype.replaceChild = function(stateDef, deep) {
    var state = new State();
    angular.extend(state, stateDef);

    return this.setChild(state, deep);
  };
  ...
  //*********************************************
  //this redefines the child in place (i.e. doesn't wipe out its children)
  State.prototype.setChild = function(state, deep) {
    if (!deep) {
  ...
  State.prototype.updateChild = function(stateDef) {
    ...
    if (!state) {
      ...
      return this.replaceChild(stateDef, true);
    }
    else {
      angular.extend(state, stateDef);

      return this.setChild(state, false);
    }
  };
  ...
  State.prototype.replaceState = function(stateDef, deep) {
    var parent = this.prepareFlatDefinitionAndGetParent(stateDef);

    parent.replaceChild(stateDef, deep);
  };
...
//***************************************
  ...
  function state(name, definition) {
    ...
    statesTree.replaceState(definition);
    return this;
  }
  this.state = state;
  ...
  function replaceChild(stateDef, deep) {
    return statesTree.replaceChild(stateDef, deep);
  }
  ...
  function replaceState(stateDef) {
    return statesTree.replaceState(stateDef);
  }
...
//***************************************
  ...
  $get.$inject = ['$rootScope', '$q', '$templateFactory', '$injector', '$stateParams', '$location'];
  this.$get = $get;
